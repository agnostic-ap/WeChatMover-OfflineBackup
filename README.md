...
已迁移状态下点「更改…」不是简单地改个记录，而是完整的数据转移流程：第一重确认（当前位置 + 后果说明）→ 选择新位置 → 第二重确认（数据大小 + 耗时提示）→ 自动退出微信后逐项 拷贝 → 校验 → 软链换指向 → 清除旧位置数据，任何一步失败都会回滚（软链指回原位置）。转移期间微信不可用，完成后自动重签名。新位置强烈建议 APFS 格式（非 APFS 会给强提示但不禁止）。
