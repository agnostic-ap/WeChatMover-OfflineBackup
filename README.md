...
还原：点击「还原到 Mac…」（红色确认）。本地 `_backup` 还在时秒还原（改名回去即可）；备份已删则从外置盘完整拷回。完成后再次重签名。

还原后外置盘上的 `<目标文件夹>/WeChatData` 会保留（安全设计）。确认不再需要后，点「清理外置数据…」手动删除：工具会先校验没有任何符号链接还指向其中（仍在使用则拒绝并提示先还原），显示占用大小并二次确认后才删除。
