...
迁移完成时会向 `<目标文件夹>/WeChatData/manifest.json` 写入清单（逐项记录迁移时刻的目录指纹：相对路径+大小+mtime 的 stat 聚合，不读文件内容）。还原外置数据前，工具据此做**新旧判定**：外置数据与本地备份指纹一致 → 直接走本地备份秒还原；不一致（通常是迁移后有新聊天记录写入外置盘）→ 弹三选项确认：「使用外置数据还原（推荐）」/「仍使用本地备份」/「取消」。无 manifest 的旧迁移会自动降级为双侧指纹对比；外置盘不可读时按原流程继续。
